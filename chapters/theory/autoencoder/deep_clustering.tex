...
\noindent The matrix elements $q_{ij}$ are then the probability of the latent sample $\mathbf{z}_i$ belonging to cluster $j$. To define the corresponding clustering loss, we first compute a target distribution $p_{ij}$, which represents the confidence of the mapping $q_{ij}$. We then define the target distribution as 
