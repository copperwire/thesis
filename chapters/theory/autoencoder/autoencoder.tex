...
\begin{figure}[tb]
	...
	\caption[Autoencoder schematic]{Schematic of an autoencoder model. A sample $\hat{\mathbf{x}}$ is compressed to a lower-dimensional representation $\mathbf{z}$, which is subsequently reconstructed to the original space by the decoder.}
...
We introduce the evidence lower bound (ELBO) as an approximation to the evidence following the derivation laid out by \cite{Kingma2013}. The EBLO function is defined as 
